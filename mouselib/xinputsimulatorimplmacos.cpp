...
{
    auto mouseType = button==1?kCGEventLeftMouseDown:kCGEventRightMouseDown;
    
    CGEventRef mouseEv = CGEventCreateMouseEvent(
                NULL, mouseType,
                CGPointMake(this->currentX, this->currentY),
                kCGMouseButtonRight); // use int buttn from parameter

    CGEventSetType(mouseEv, mouseType);
    CGEventPost(kCGHIDEventTap, mouseEv);
...
{
    auto mouseType = button==1?kCGEventLeftMouseUp:kCGEventRightMouseUp;
...
void XInputSimulatorImplMacOs::mouseDoubleClick(int clickCount)
{
    CGEventRef theEvent = CGEventCreateMouseEvent(NULL, kCGEventLeftMouseDown, CGPointMake(this->currentX,this->currentY), kCGMouseButtonLeft);  
